...
class Cuts:
    ...
    def getCutWord(self, mcname):
        word = ''
        for branch in self.branch_info.branch_names:
            if self.branch_info.toggle_cuts[branch] == 1:
    ...
    def appendToCutWord(self, word, branchname, mcname):
        if mcname == '85':
            if word == '':
                word = word + branchname + '>' + str(self.branch_info.branch_85_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_85_highcut[branchname])
            else:
                word = word + '&&' + branchname + '>' + str(self.branch_info.branch_85_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_85_highcut[branchname])
        if mcname == '90':
            if word == '':
                word = word + branchname + '>' + str(self.branch_info.branch_90_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_90_highcut[branchname])
            else:
                word = word + '&&' + branchname + '>' + str(self.branch_info.branch_90_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_90_highcut[branchname])
        if mcname == '95':
            if word == '':
                word = word + branchname + '>' + str(self.branch_info.branch_95_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_95_highcut[branchname])
            else:
                word = word + '&&' + branchname + '>' + str(self.branch_info.branch_95_lowcut[branchname]) + '&&' + branchname + '<' + str(self.branch_info.branch_95_highcut[branchname])
        return word
